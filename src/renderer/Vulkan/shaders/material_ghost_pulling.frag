...
// Restored glsl/GhostPulling.glsl semantics.  See the vertex module for the
...
//   0 scrollX, 1 scrollY, 2 sizeX, 3 sizeY, 4 distortionLength.
...
void main() {
    // Retain the transform vocabulary used by Raven's shipped displacement
    // family.  GhostPulling's authored value (50 in the stock test material)
    // acts as a length divisor, producing a bounded bidirectional pull.
    vec2 newSize = vec2(
    ...
    vec2 direction =
        texture(DisplacementMap, displacementCoord).xy * 2.0 - vec2(1.0);
    float distortionLength =
        max(abs(material.shaderParms[4].x), 1.0e-4);
    vec4 color =
        texture(Image, vTexCoord + direction / distortionLength) * vColor;
