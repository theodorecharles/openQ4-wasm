...
{
	rvGEMaskEdit* edit = reinterpret_cast< rvGEMaskEdit * >( GetWindowLongPtr( hWnd, GWLP_USERDATA ) );
...
void MaskEdit_Attach ( HWND hWnd, const char* invalid )
{
	...
	edit->mProc = reinterpret_cast< WNDPROC >( GetWindowLongPtr( hWnd, GWLP_WNDPROC ) );
	strcpy ( edit->mInvalid, invalid );
	SetWindowLongPtr( hWnd, GWLP_USERDATA, reinterpret_cast< LONG_PTR >( edit ) );
	SetWindowLongPtr( hWnd, GWLP_WNDPROC, reinterpret_cast< LONG_PTR >( MaskEdit_WndProc ) );
}
