...
{
	SetWindowLongPtr( hWnd, GWLP_USERDATA, static_cast< LONG_PTR >( inc * 100.0f ) );
}
...
void SpinButton_HandleNotify ( NMHDR* hdr )
{
	...
	float inc = static_cast< float >( GetWindowLongPtr( hdr->hwndFrom, GWLP_USERDATA ) );
	if ( inc == 0 )
	{
		inc = 100.0f;
	...
	LONG minRange;
	LONG maxRange; 
	SendMessage( hdr->hwndFrom, UDM_GETRANGE32,
		reinterpret_cast< WPARAM >( &minRange ), reinterpret_cast< LPARAM >( &maxRange ) );
