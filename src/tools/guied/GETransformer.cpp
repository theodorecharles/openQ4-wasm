...
{
	rvGETransformer* trans = reinterpret_cast< rvGETransformer * >( GetWindowLongPtr( hWnd, GWLP_USERDATA ) );
	...
	switch ( msg )
	{
		...
		case WM_CREATE:
		{
			...
			SetWindowLongPtr( hWnd, GWLP_USERDATA, reinterpret_cast< LONG_PTR >( trans ) );
