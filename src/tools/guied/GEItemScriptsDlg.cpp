...
{
	WNDPROC wndproc = reinterpret_cast< WNDPROC >( GetWindowLongPtr( hwnd, GWLP_USERDATA ) );
...
bool GEItescriptsDlg_Init ( HWND hwnd )
{
	...
	window = reinterpret_cast< idWindow * >( GetWindowLongPtr( hwnd, GWLP_USERDATA ) );
	assert ( window );
	...
	SetWindowLongPtr( script, GWLP_WNDPROC, reinterpret_cast< LONG_PTR >( GEScriptEdit_WndProc ) );
